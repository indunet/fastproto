...
* **ROS 2（可选）：** 支持离线 CDR 编解码与 rosbag2 录制文件读取。
...
FastProto 还提供 **可选的、只读** ROS 2 模块，用于 Java 离线数据处理。它们 **不包含** 在默认 `fastproto` 制品中，也 **不会** 连接 DDS 或作为 ROS 2 节点运行。
...
指南：[ROS 2 消息](https://indunet.github.io/fastproto/help/ros2-messages) · [Bag 读取](https://indunet.github.io/fastproto/help/ros2-bag)
