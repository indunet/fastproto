...
public class UInt16ArrayCodecTest {
    ...
    public void testDecode1() {
        ...
        val expected = IntStream.range(0, 10)
                .map(i -> random.nextInt(UInt16Type.MAX_VALUE))
                .toArray();

        val bytes = BinaryUtils.uint16Of(expected, ByteOrder.LITTLE);
    ...
    public void testEncode1() {
        ...
        IntStream.range(0, 16)
                .forEach(i -> values[i] = random.nextInt(UInt16Type.MAX_VALUE));
    ...
    protected CodecContext mock(int offset, int length, ByteOrder order) {
        return CodecContext.builder()
                .dataTypeAnnotation(AnnotationUtils.mock(UInt16ArrayType.class, offset, length, order))
                .build();
    }
}
