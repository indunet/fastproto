...
public class Int64ArrayCodecTest {
    ...
    public void testEncode2() {
        ...
        assertThrows(NullPointerException.class,
                () -> this.codec.encode(mock(0, -1, ByteOrder.LITTLE), (ByteBufferOutputStream) null, new long[8]));
        assertThrows(NullPointerException.class,
                () -> this.codec.encode(mock(0, -1, ByteOrder.LITTLE), new ByteBufferOutputStream(bytes), null));
        assertThrows(EncodingException.class,
                () -> this.codec.encode(mock(-2, -7, ByteOrder.LITTLE), new ByteBufferOutputStream(bytes), new long[8]));
        assertThrows(EncodingException.class,
                () -> this.codec.encode(mock(-11, -7, ByteOrder.LITTLE), new ByteBufferOutputStream(bytes), new long[8]));
        assertThrows(EncodingException.class,
                () -> this.codec.encode(mock(10, -1, ByteOrder.LITTLE), new ByteBufferOutputStream(bytes), new long[8]));
        assertThrows(EncodingException.class,
                () -> this.codec.encode(mock(0, 11, ByteOrder.LITTLE), new ByteBufferOutputStream(bytes), new long[8]));
    }

    protected CodecContext mock(int offset, int length, ByteOrder order) {
        return CodecContext.builder()
                .dataTypeAnnotation(AnnotationUtils.mock(Int64ArrayType.class, offset, length, order))
                .build();
    }
}
