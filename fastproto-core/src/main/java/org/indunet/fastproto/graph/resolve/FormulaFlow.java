...
import org.indunet.fastproto.formula.FormulaRegistry;
 ...
 *
 * <p>This class looks up pre-compiled formulas from the {@link FormulaRegistry} generated by the annotation processor.
 * Lambda expressions in annotations require the fastproto-processor dependency to generate formula classes at compile time.
 *
 * @author Deng Ran
 * @since 3.5.0
 */
public class FormulaFlow extends ResolvePipeline {
    ...
    private void setDecodingFormula(Reference reference, DecodingFormula formula) {
        if (formula.value().length != 0) {
            // Option 1: Explicit Function class specified
            ...
            // Option 2: Lambda expression - requires annotation processor to generate formula class
            Function<?, ?> registeredFormula = FormulaRegistry.get(reference.getField(), "decoding");
            if (registeredFormula != null) {
                reference.setDecodingLambda(registeredFormula);
            } else {
                throw new DecodingException(String.format(
                        "No pre-compiled formula found for field %s. " +
                        "Please add fastproto-processor dependency to generate formula classes at compile time, " +
                        "or use @DecodingFormula(value = YourFunction.class) with a precompiled Function class.",
    ...
    private void setEncodingFormula(Reference reference, EncodingFormula formula) {
        if (formula.value().length != 0) {
            ...
            // Option 2: Lambda expression - requires annotation processor to generate formula class
            Function<?, ?> registeredFormula = FormulaRegistry.get(reference.getField(), "encoding");
            if (registeredFormula != null) {
                reference.setEncodingLambda(registeredFormula);
            } else {
                throw new EncodingException(String.format(
                        "No pre-compiled formula found for field %s. " +
                        "Please add fastproto-processor dependency to generate formula classes at compile time, " +
                        "or use @EncodingFormula(value = YourFunction.class) with a precompiled Function class.",
                        reference.getField().toString()));
            }
        } else {
