...
/**
 * Unit test of StringBuffer type codec.
 *
 * @author Deng Ran
 * @since 3.2.1
 */
public class StringBufferCodecTest {
    StringBufferCodec codec = new StringBufferCodec();

    public static List<Arguments> testDecode1() {
        return Stream.of(
                Arguments.arguments("ABCabc".getBytes(), 0, -1, StandardCharsets.UTF_8, new StringBuffer("ABCabc")),
                Arguments.arguments("abcdef".getBytes(), 3, 3, StandardCharsets.UTF_8, new StringBuffer("def")),
                Arguments.arguments("abcdef".getBytes(), -3, 3, StandardCharsets.UTF_8, new StringBuffer("def"))
    ...
    @ParameterizedTest
    @MethodSource
    public void testDecode1(byte[] datagram, int byteOffset, int length, Charset set, StringBuffer actual) {
    ...
    @ParameterizedTest
    @MethodSource
    public void testEncode1(byte[] datagram, int byteOffset, int length, Charset set, StringBuffer value, byte[] actual) {
    ...
    public void testEncode2() {
        ...
        assertThrows(NullPointerException.class,
                () -> this.codec.encode(null, 0, 8, StandardCharsets.UTF_8, new StringBuffer("ABC")));
        assertThrows(EncodingException.class,
                () -> this.codec.encode(datagram, -1, 2, StandardCharsets.UTF_8, new StringBuffer("ABC")));
        assertThrows(EncodingException.class,
                () -> this.codec.encode(datagram, 10, -1, StandardCharsets.UTF_8, new StringBuffer("ABC")));
        assertThrows(EncodingException.class,
                () -> this.codec.encode(datagram, 2, 10, StandardCharsets.UTF_8, new StringBuffer("ABC")));
    }
}
